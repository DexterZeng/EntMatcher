...
def get_matrix(triples, entity, rel):
    ...
    for h, t, r in sorted(radj, key=lambda x: x[0] * 10e10 + x[1] * 10e5):
        if ' '.join([str(h), str(t)]) in s:
            r_index.append([count, r])  # 一个关系可能在多个ht对中出现，而一个ht对可能存在多个关系，因此通过count,r来进行标记
            r_val.append(1)
            d[count] += 1
        else:
            count += 1
    ...
    for i in range(len(r_index)):
        r_val[i] /= d[r_index[i][0]]  # 若一个ht对里面存在多个r,那么r的importance就下降了
...
def load_data(lang):
    ...
    dev_pair = load_alignment_pair(lang + 'val_ent_ids')
